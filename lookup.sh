...
root@class:/var/www/html/rainbow# cat lookup.sh
#!/bin/bash

if ! [[ "$1" =~ ^[0-9A-Fa-f]{1,}$ ]] ; then
    echo "Not a valid hash! Please submit a valid hash!"
    exit
fi
...
# 0 is LM, 1 is MD5, 2 is SHA1, 3 is NTLM
if [ $2 -eq 0 ]; then
    DIR="/rt/lanman"
elif [ $2 -eq 1 ]; then
    DIR="/rt/md5"
...
sshpass -e ssh rain@192.168.100.2 rcrack $DIR -h "$1"
